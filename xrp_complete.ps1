...
function Get-K {
    ...
    $r = Call-API -ep "/v5/market/kline" -q "category=spot&symbol=XRPUSDT&interval=$interval&limit=$limit"
...
for ($i = $win.per + 20; $i -lt $closes.Count - 3; $i++) {
    ...
    $dow[$day].total++; if ($won) { $dow[$day].wins++ } else { $dow[$day].losses++ }
}
